            ...
            <div className="flex flex-wrap justify-center gap-4 mt-12">
              {categories.map(cat => (
                <button
                  ...
                  className={`flex items-center gap-2 px-8 py-3 rounded-full font-bold text-sm transition-all ${activeCategory === cat.name ? 'bg-[#064E3B] text-white shadow-lg' : 'bg-gray-50 text-gray-500 hover:bg-gray-100'
                    }`}
                >
                  {cat.icon} {cat.name}
                </button>
          ...
          <div className="bg-emerald-50/50 rounded-[40px] p-8 md:p-12 mb-16">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {allTreatments
                ...
                .map(t => (
                  <div key={t.id} className="bg-white rounded-[32px] overflow-hidden border border-gray-100 shadow-sm hover:shadow-xl transition-all group">
                    ...
                    <div className="p-8">
                      ...
                      <p className="text-gray-400 text-sm mb-4 font-medium line-clamp-2">{t.desc}</p>

                      {t.recommendedFor && (
                        <div className="mb-4">
                          <p className="text-emerald-900 font-bold text-[10px] uppercase tracking-widest mb-1">Recommended For</p>
                          <p className="text-xs text-gray-600 italic">{t.recommendedFor}</p>
                        </div>
                      )}

                      <div className="flex flex-wrap gap-2 mb-6">
                        {t.benefits.slice(0, 3).map((b: string) => (
                          <span key={b} className="bg-gray-50 text-gray-500 px-2 py-1 rounded-md text-[9px] font-bold uppercase border border-gray-100">
                            {b}
                          </span>
                        ))}
                      </div>
              ...
              {allTreatments.filter(t => {
                ...
                return t.cat === activeCategory;
                  ...
                  <div className="col-span-full text-center py-12">
                    <p className="text-gray-400 font-medium">No treatments found in this category yet.</p>
                  </div>
                )}
            </div>
          </div>
            ...
            {herbs.map(herb => (
              <div key={herb.id} className="bg-white rounded-[40px] overflow-hidden border border-gray-100 shadow-sm hover:shadow-xl transition-all group flex flex-col text-left">
                ...
                <div className="p-8 flex-1 flex flex-col">
                  ...
                  <p className="text-[11px] font-serif italic text-gray-400 mb-4">{herb.botanicalName}</p>
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1">
                        {herb.keyBenefits.slice(0, 3).map((b: string) => (
                          <span key={b} className="bg-gray-50 text-gray-500 px-2 py-0.5 rounded text-[9px] font-bold uppercase border border-gray-100">
                            {b}
                          </span>
                        ))}
                      </div>
